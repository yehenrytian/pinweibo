...
function newWeiboBoxReset()
   {
   document.streamform.newweiboboxpop.value = "What is happening?";
   document.streamform.remain.value = 140;   
   }
function tweetPopUpBoxReset()
   {
   document.streamformpop.happeningpopup.value = "有什么新鲜事告诉大家?";
...
function loadimg(fieldId){
   ...
   document.getElementById(id).innerHTML = "<span style='font-size:bold;'>正在加载 ...</span><img src='../images/loading.gif' style='width:auto; height:auto;'/>";
}
...
function streamAjaxPostWeiboPop()
   {
   var xmlhttp = GetXmlHttpObject();
   if (xmlhttp == null)
      {
   ...
   if (!verify(document.streamformpop))
      return;
   
   var post = unescape(document.streamformpop.happeningpopup.value);
   
   var parms = "&weibo=true";
   ...
   //alert("I am here22! " + url);
   url = url + "&sid=" + Math.random();
   xmlhttp.onreadystatechange=function(){stateChangedPopUpRepost(xmlhttp, 1)}
   xmlhttp.open("GET", url, true);
   xmlhttp.send(null);
   }
...
// cObj.style.filter="progid:DXImageTransform.Microsoft.Alpha(style=3,opacity=25,finishOpacity=75";
 ...
 var whratio = iObj.width / iObj.height;
 
 if (iObj.height > cHeight && iObj.width > cWidth)
    {
	iObj.height = cHeight;
	iObj.width = cWidth;	
    }
 else if (iObj.height > cHeight)
    {
	iObj.height = cHeight;
	iObj.width = cHeight * whratio; 	
	}
 else if (iObj.width > cWidth)
    {
	iObj.width = cWidth;
	iObj.height = cWidth / whratio;
    }
...
function commentPopUp(userPic, oriPost, rePost, postId, oriPostId){
 ...
 textAreaDiv.innerHTML = '<div id="columns"><ul id="column2" class="column" style="width:100%;">' + 
						   ...
						   '<div class="widget-head"><h3>添加微博评论</h3></div>' + 
						   ...
						   '<div align="right"><span class="notice">还可以输入:</span>' +
						   '<input class="remainpopup" disabled name="remain" value=140></div>' +
						   '<br/><div class="notice" align="left"><input type="checkbox" name="alsorepost" value="1"/>同时转发到我的微博</div>' +
						   ((oriPostId != 0) ? '<div class="notice" align="left"><input type="checkbox" name="commentori" value="1"/>同时评论给原文作者</div>' : '') +
						   '<br/><div align="center"><input class="button streamit" type="button" value="评论" onclick="' +
						   ...
						   '<input class="button reset" type="button" value="取消" onclick="document.getElementById(\'close\').onclick();"/></div></form></div></li></ul></div>';
 ...
 closeObj.style.left = "50%"
 closeObj.style.top = "50%"
 closeObj.style.marginLeft = (185) + "px";
 closeObj.style.marginTop = -225 + "px";
...
function repostPopUp(userPic, oriPost, rePost, postId, oriPostId){
 ...
 textAreaDiv.innerHTML = '<div id="columns"><ul id="column2" class="column" style="width:100%;">' + 
                           '<li class="widget color-red" style="margin: 5px 0px 0px 0px; padding:0px">' + 
						   '<div class="widget-head"><h3>转发此微博</h3></div>' + 
						   '<div class="widget-content" style="padding-bottom:10px;" align="center">' +
                           '<img class="grid-item-avatar" style="width:48px;height:48px;margin-top:10px;" src="' + userPic +'">' +
						   ...
						   '<div align="right"><span class="notice">还可以输入:</span>' +
						   '<input class="remainpopup" disabled name="remain" value=140></div>' +
						   '<br/><div class="notice" align="left"><input type="checkbox" name="alsocomment" value="1"/>同时评论给作者</div>' +
						   ((oriPostId != 0) ? '<div class="notice" align="left"><input type="checkbox" name="commentori" value="1"/>同时评论给原文作者</div>' : '') +
						   '<br/><div align="center"><input class="button streamit" type="button" value="转发" onclick="' +
...
// popup compose new weibo
function newWeiboPopUp(){
 var cObj = document.createElement("div");
 cObj.setAttribute("id","cdiv");
 cObj.setAttribute("name","cdiv");
 cObj.setAttribute("class","popupwrapper");
 ...
 textAreaDiv.innerHTML = '<div id="columns"><ul id="column2" class="column" style="width:100%;">' + 
                           '<li class="widget color-red" style="margin: 5px 0px 0px 0px; padding:0px">' + 
						   '<div class="widget-head"><h3>发布新微博</h3></div>' + 
						   '<div class="widget-content" style="padding-bottom:10px;" align="center">' +
                           '<form method="post" enctype="multipart/form-data" name="streamformpop" id="streamformpop">' +
						   '<div id="streamboxpop" name="streamboxpop" class="tweetbox">' +
						   '<textarea id="happeningpopup" class="tweetarea" oninput="textcount(this, 140, this.form.remain);" onfocus="if (this.value == \'有什么新鲜事告诉大家?\') this.value=\'\';textcount(this, 140, this.form.remain);" onblur="if (this.value == \'\') this.value=\'有什么新鲜事告诉大家?\';textcount(this, 140, this.form.remain);" onkeydown="textcount(this, 140, this.form.remain);" onkeyup="textcount(this, 140, this.form.remain);">有什么新鲜事告诉大家?</textarea></div><br/>' +
						   '<img class="grid-item-avatar" src="../images/weibo48x48.png" alt="weibo" style="width:24px;height:24px;"/><span class="notice" style="float:left;">新浪微博</span>' +
						   '<div align="right"><span class="notice">还可以输入:</span>' +
						   '<input class="remainpopup" disabled name="remain" value=140></div>' +
						   '<br/><div align="center"><input class="button reset" type="button" value="重写" onclick="tweetPopUpBoxReset()"/><input class="button streamit" type="button" value="发布" onclick="streamAjaxPostWeiboPop();"/>' +
						   '<input class="button reset" type="button" value="取消" onclick="document.getElementById(\'close\').onclick();"/></div></form></div></li></ul></div>';
...
function stateChangedPopUpRepost(xmlhttp, type)
   ...
   if (xmlhttp.readyState == 4 || xmlhttp.readyState == "complete")
	  ...
	  if (type == 4)
         document.getElementById('streamboxpop').innerHTML = "<span class='notice'>正在处理...</span><img src='../images/loading.gif' style=\"width=:220px;height:19px\"/>";
	  else if (type == 1)
	     document.getElementById('streamboxpop').innerHTML = "<span class='notice'>正在发布...</span><img src='../images/loading.gif' style=\"width=:220px;height:19px\"/>";
	  }
   }   
...
function masonryContainer(id){  
  var wall = new Masonry( document.getElementById('container' + id) );
  }  